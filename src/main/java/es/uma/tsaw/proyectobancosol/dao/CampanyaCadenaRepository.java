 ...
 * @author bancosol
 */
public interface CampanyaCadenaRepository extends JpaRepository<CampanyaCadenaEntity, CampanyaCadenaId> {

    /**
     * Encuentra todas las cadenas que participan en una campaña
     */
    @Query("SELECT cc FROM CampanyaCadenaEntity cc WHERE cc.campanya.idCampanya = :idCampanya")
     ...
     */
    @Query("SELECT cc FROM CampanyaCadenaEntity cc WHERE cc.cadena.idCadena = :idCadena")
    List<CampanyaCadenaEntity> findByCadena(@Param("idCadena") Integer idCadena);

    /**
     * Verifica si una cadena participa en una campaña
     */
    @Query("SELECT COUNT(cc) > 0 FROM CampanyaCadenaEntity cc WHERE cc.campanya.idCampanya = :idCampanya AND cc.cadena.idCadena = :idCadena")
